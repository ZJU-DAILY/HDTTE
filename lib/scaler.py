...
class StandardScaler(object):
    ...
    def inverse_transform(self, data,tensor=True):
        if tensor:
            mean = self.mean
            std = self.std
            if std == 0:
...
class StaticFeatureScaler(object):
    def __init__(self):
        features_name = ['fes_'+str(i+1) for i in range(6)]
        ...
        features_type = {"fes_1": {"float": 1}, "fes_2": {"one_hot": 6, "start": 1}, "fes_3": {"float": 1},
                          "fes_4": {"one_hot": 7, "start": 1}, "fes_5": {"binary": 1},"fes_6": {"binary": 1}}
